...
impl Connections {
    ...
    pub fn remove_connection(&self, node_pk: PublicKey) -> IoFuture<()> {
        if let Some(connection) = self.connections.write().remove(&node_pk) {
            ...
            let futures = connection.connections
                .into_iter()
                .flat_map(|relay_pk| relays.get(&relay_pk).into_iter())
    ...
    fn main_loop(&self) -> IoFuture<()> {
        ...
        relays.retain(|_, relay| if relays_len > RECOMMENDED_FRIEND_TCP_CONNECTIONS && relay.connections_count() == 0 {
            relays_len -= 1;
            relay.disconnect();
            false
        } else {
